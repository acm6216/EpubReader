...
@BindingAdapter(
    ...
    "paddingBottomFromTopSystemWindowInsets",
    requireAll = false
)
fun View.applySystemWindowInsetsPadding(
    ...
    doOnApplyWindowInsets { view, insets, padding, _ ->
        ...
        val bottomFromTop = if(applyBottomFromTop) systemBars.top else 0

        view.setPadding(
            padding.left + left,
            padding.top + top,
            padding.right + right,
            padding.bottom + if(applyBottomFromTop) bottomFromTop else bottom
        )
    }
}
